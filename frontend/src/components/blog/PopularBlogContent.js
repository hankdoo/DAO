...
const PopularBlogContent = (props) => {
    ...
    const PopularBlogCards = popularPostData.slice(0, 7).map((elem, index) => (
        <div key={index} className="popular-blog">
            <Link className="post-title mb-2" href={`${process.env.PUBLIC_URL}/blog-details/${elem.id}`} >
                <span><i className="bi bi-lightning-fill" /></span>
                <h5 className="hover-primary">{elem.title}</h5>
            </Link>
            <p>{elem.excerpt}</p>
            <Link className="btn btn-minimal text-start hover-primary" href={`${process.env.PUBLIC_URL}/blog-details/${elem.id}`} >
                Continue Reading 
