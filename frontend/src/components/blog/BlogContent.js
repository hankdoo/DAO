        ...
        <div key={index} className="col-12 col-lg-6 col-xxl-4">
            <div className="card blog-card border-0 bg-transparent bg-transparent">
                <div className="img-wrap">
                    <Link href={`${process.env.PUBLIC_URL}/blog-details/${elem.id}`} >
                        <img src={`${process.env.PUBLIC_URL}/${elem.image}`} alt={elem.title} />
                    </Link>
                </div>
                <div className="card-body p-3">
                    <Link className="post-title mb-2 hover-primary" href={`${process.env.PUBLIC_URL}/blog-details/${elem.id}`} >
                        {elem.title}
                    </Link>
                    <p className="mb-4">{elem.excerpt}</p>
                    <Link className="btn btn-primary btn-sm rounded-pill" href={`${process.env.PUBLIC_URL}/blog-details/${elem.id}`} >
                        Read more
