...
export default function PrivateRound({ account }: IProps) {
  ...
  useEffect(() => {
    const init = async () => {
      if (account?.web3Provider && account.wallet) {
        ...
        Promise.all([
          await privateRoundContract.getBalanceByAddress(account?.wallet?.address),
          await privateRoundContract.getTgeStart(),
          await privateRoundContract.getStartTime(),
          await privateRoundContract.getEndTime(),
          await privateRoundContract.getMin(),
          await privateRoundContract.getMax(),
          await privateRoundContract.getBalanceOfSeed(),
          await privateRoundContract.getTotalTokensSold(),
          await privateRoundContract.getTge(),
          await privateRoundContract.getCost(),
          await privateRoundContract.getCapacity(),
        ]).then((values) => {
